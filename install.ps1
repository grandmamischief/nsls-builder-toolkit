...
"@
    try {
        ...
        Write-TextNoBom -Path (Join-Path $ShimDir 'nsls-python.cmd') -Content $shim
...
if (Test-Path $PyExe) {
    ...
    $shimOk = Test-Path (Join-Path $env:LOCALAPPDATA 'Programs\nsls-bin\nsls-python.cmd')
    if ($pyVer -and $docxOk -and $pptxOk -and $shimOk) {
        ...
        if (-not $shimOk) { $PrereqReport += "  [warn] nsls-python launcher missing - the document skills call it. Re-run this installer." }
    } else {
